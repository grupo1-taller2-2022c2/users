...
@router.get(
    "/{useremail}", response_model=DriverProfile, status_code=status.HTTP_200_OK
)
...
@router.get(
    "/me/{useremail}", response_model=DriverSelfProfile, status_code=status.HTTP_200_OK
)
def get_user(useremail: str, db: Session = Depends(get_db)):
    user_db = get_user_by_email(useremail, db)
    if not user_db:
...
def update_driiver_profile(
    useremail: str, user: DriverProfile, db: Session = Depends(get_db)
):
    user_db = get_user_by_email(useremail, db)
    if not user_db:
        raise HTTPException(status_code=404, detail="The user doesn't exist")
...
def user_add_driver_ratings(rating: DriverRating, db: Session = Depends(get_db)):
    ...
    if not db_user:
        raise HTTPException(status_code=404, detail="The user doesn't exist")
    return add_driver_rating(
        rating.driver_email, rating.trip_id, rating.ratings, rating.message, db
    )
...
def report_driver(report: DriverReport, db: Session = Depends(get_db)):
    ...
    if not db_passenger:
        raise HTTPException(status_code=404, detail="The passenger doesn't exist")
